...
pub fn bundled_catalog() -> &'static [CatalogSeedRow] {
    &[
        CatalogSeedRow {
            id: "lit-protocol",
            name: "Lit Protocol",
            short_description: "PKP agent wallet with decentralized policy enforcement",
            long_description: "Create a PKP agent wallet via Lit's distributed MPC network. Policies (spend limits, protocol whitelists) are enforced by Lit TEE nodes before signing. SHADOW owns the UI; Lit provides decentralized key management and policy enforcement.",
            icon_key: "Zap",
            version: "1.0.0",
            author: "SHADOW (bundled)",
            features_json: r#"["PKP wallet creation via EthWallet auth","Lit Action policy enforcement on TEE nodes","Distributed MPC signing","Precheck before execution"]"#,
            permissions_json: r#"["agent_wallet.manage","execution.precheck","network.lit"]"#,
            secret_requirements_json: r#"[]"#,
            agent_tools_json: r#"["lit_protocol_wallet_status","lit_protocol_precheck_action","lit_protocol_connect_wallet","lit_protocol_execute_swap"]"#,
            network_scopes_json: r#"["lit-protocol"]"#,
        },
        CatalogSeedRow {
            id: "flow",
            name: "Flow",
            short_description: "Cadence Flow reads, signing, schedules, and bridge previews",
            long_description: "Cadence-native portfolio reads via Flow access nodes; Flow EVM assets use Alchemy. Session-unlocked signing in the isolated apps runtime submits minimal Cadence txs and logs schedule/cancel intents. Flow Actions and Cross-VM bridge tooling are preview-first (testnet recommended).",
            icon_key: "Waves",
            version: "1.0.0",
            author: "SHADOW (bundled)",
            features_json: r#"["Account connectivity","Cadence signing (session key, sidecar)","Schedule/cancel intent logging","Flow Actions preview","Cross-VM bridge preview","SHADOW scheduler jobs"]"#,
            permissions_json: r#"["flow.account.read","flow.tx.prepare","network.flow"]"#,
            secret_requirements_json: r#"[]"#,
            agent_tools_json: r#"["flow_protocol_account_status","flow_protocol_prepare_sponsored_transaction","flow_schedule_transaction","flow_setup_recurring","flow_list_scheduled","flow_cancel_scheduled","flow_estimate_schedule_fee","flow_compose_defi_action","flow_bridge_tokens"]"#,
